...
def window_size():
    ...
    # (it is the same as window_height = window_height - number_of_lines_for_input)
...
def render_screen(is_center_horizontal_all, *sentences):
    ...
    vertical_padding_before_divided = ws["height"] - \
        number_of_lines_in_sentences
        ...
        # other lines
            ...
            if is_center_horizontal_all:
                ...
                horizontal_padding = int(
                    (horizontal_padding_before_divided - 1 if is_odd else horizontal_padding_before_divided) / 2)
                print("#", end="")
                print(" "*(horizontal_padding - 1), end="")
                print(sentence, end="")
                print(
                    " "*(horizontal_padding if is_odd else horizontal_padding - 1), end="#\n")
            else:
                if number_of_character_longest_sentences is None:
                    number_of_character_longest_sentences = len(
                        max(sentences, key=len))
                    horizontal_padding_before_divided = ws["width"] - \
                        number_of_character_longest_sentences
                    is_odd = False if horizontal_padding_before_divided % 2 == 0 else True
                    horizontal_padding = int(
                        (horizontal_padding_before_divided - 1 if is_odd else horizontal_padding_before_divided) / 2)
                print("#", end="")
                print(" "*(horizontal_padding - 1), end="")
                print(sentence, end="")
                print(" "*(ws["width"] - horizontal_padding -
                      len(sentence) - 1), end="#\n")
